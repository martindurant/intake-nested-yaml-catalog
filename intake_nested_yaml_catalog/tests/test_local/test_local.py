...
def test_local_catalog(catalog1):
    assert_items_equal(list(catalog1.level1.level2),
                       ['use_example1', 'nested', 'entry1', 'entry1_part',
                        'remote_env', 'local_env', 'text', 'arr', 'datetime'])
    assert catalog1.level1.level2['entry1'].describe() == {
        'name': 'entry1',
        'container': 'dataframe',
    ...
    assert catalog1.level1.level2['entry1_part'].describe() == {
        'name': 'entry1_part',
        'container': 'dataframe',
        'user_parameters': [
            {
    ...
    assert catalog1.level1.level2['entry1'].get().container == 'dataframe'
    md = catalog1.level1.level2['entry1'].get().metadata
    md.pop('catalog_dir')
    assert md == dict(foo='bar', bar=[1, 2, 3])

    # Use default parameters
    assert catalog1.level1.level2['entry1_part'].get().container == 'dataframe'
    # Specify parameters
    assert catalog1.level1.level2['entry1_part'].get(part='2').container == 'dataframe'


def test_get_items(catalog1):
    for key, entry in catalog1.level1.level2.items():
        assert catalog1.level1.level2[key].describe() == entry.describe()


def test_nested(catalog1):
    assert 'nested' in catalog1.level1.level2
    assert 'entry1' in catalog1.level1.level2.nested.level1.level2
    assert catalog1.level1.level2.entry1.read().equals(catalog1.level1.level2.nested.level1.level2.entry1.read())
...
def test_nested_gets_name_from_super(catalog1):
    ...
    assert 'nested' in catalog1.level1.level2
    nested = catalog1.level1.level2.nested
    assert nested.name == 'nested'
    assert nested().name == 'nested'


def test_hash(catalog1):
    assert catalog1.level1.level2.nested() == catalog1.level1.level2.nested.level1.level2.nested()
...
def test_use_source_plugin_from_config(catalog1):
    catalog1['level1.level2.use_example1'].get()


def test_entry_dir_function(catalog1):
    assert 'level1' in dir(catalog1.level1.level2.nested)
    assert 'nested' in dir(catalog1.level1.level2.nested.level1.level2)
...
#     cat = open_nested_yaml_cat([fn1, fn2])
#     assert cat.name == '2 files'
#     assert cat.description == 'Catalog generated from 2 files'
#
#     cat = open_nested_yaml_cat([fn1, fn2], name='special_name',
...
def test_cat_with_declared_name():
    ...
    cat = open_nested_yaml_cat(fn, name='name_in_func', description=description)
...
def test_cat_with_no_declared_name_gets_name_from_dir_if_file_named_catalog():
    ...
    cat = open_nested_yaml_cat(fn, name='name_in_func', description='Description in func')
    assert cat.name == 'name_in_func'
...
def test_dot_names():
    ...
    assert cat['level1.selfdot.dot', 'level1.leafdot.dot']._description == 'leaf-dot'

    assert cat['level1.self.level1.selfdot.dot', 'level1.leafdot.dot']._description == 'leaf-dot'
    assert cat['level1.self.level1.self.dot', 'level1.leafdot.dot']._description == 'leaf-dot'
    assert cat['level1.self.level1.self.dot', 'level1.leaf']._description == 'leaf'
    assert cat['level1.self.level1.self.dot', 'level1.leaf.dot']._description == 'leaf-dot'
...
def test_listing(catalog1):
    assert list(catalog1) == list(catalog1.level1.level2.nested)
    with pytest.raises(ValueError):
        list(catalog1.level1.level2.arr)
